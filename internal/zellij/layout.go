...
//	|          |                                          |  (36%)    |
//	+----------+------------------------------------------+-----------+
//	| Event Log | Evals |  OB1  |        TG Viz        | LazyGit |
//	|  (14%)    | (14%) | (18%) |        (38%)         |  (16%)  |
//	+-----------+-------+-------+----------------------+---------+
//
// Top row: 67% height — left column (prompt+fp, 10%) | agent (67%, borderless) | right column (Agents+Jira, 23%)
// Bottom row: 33% height — Event Log | Evals | OB1 | TG Viz (wide, passive overlay target) | LazyGit
//
...
func GenerateLayout(opts LayoutOpts) string {
	...
	calcurseWrapperPath := filepath.Join(projectDir, ".computecommander", "scripts", "calcurse-wrapper.sh")
	if opts.SystemWide {
		...
		calcurseWrapperPath = filepath.Join(home, ".computecommander", "scripts", "calcurse-wrapper.sh")
	}
...
%s
            ...
            pane split_direction="vertical" size="33%%" {
                pane name="Event Log" size="14%%" {
                    command "%s"
                    args "feed" "--pane"%s
                }
                pane name="Evals" size="14%%" {
                    command "%s"
                    args "evals" "--pane"%s
                }
                pane name="OB1" size="18%%" {
                    command "%s"
                    args "openbrain" "--pane"%s
                }
                pane name="TG Viz" size="38%%" {
                    command "tail"
                    args "-f" "/dev/null"
                }
                pane size="16%%" {
...
//
// The agent pane occupies the full 67% top section of the central column.
// TG Viz is placed on the bottom row alongside Event Log, Evals, OB1, and LazyGit;
// the trustgraph-viewer Electron overlay snaps to whichever pane is named "TG Viz".
...
func buildAgentPane(agentCmd, wrapperPath, tabHash string) string {
	...
	if wrapperPath != "" {
		return fmt.Sprintf("                pane name=\"Agent\" size=\"67%%\" focus=true borderless=true {\n                    command \"bash\"\n                    args \"%s\" \"%s\"\n                }", wrapperPath, tabHash)
	}

	if agentCmd == "" {
		return "                pane name=\"Agent\" size=\"67%\" focus=true borderless=true"
	}
...
// WriteCalcurseWrapper generates a wrapper script that cleans up stale
// .calcurse.pid lock files before launching calcurse. Calcurse uses a PID file
// to prevent multiple instances; when the dashboard tab is recreated (or zellij
// restarts), the old PID file remains and calcurse exits immediately.
// The wrapper checks whether the PID is still alive and removes the lock if not.
func WriteCalcurseWrapper(scriptBaseDir string) (string, error) {
	scriptDir := filepath.Join(scriptBaseDir, ".computecommander", "scripts")
	if err := os.MkdirAll(scriptDir, 0o755); err != nil {
		return "", fmt.Errorf("create scripts dir: %w", err)
	}

	scriptPath := filepath.Join(scriptDir, "calcurse-wrapper.sh")
	content := `#!/bin/bash
# Auto-generated by cmdr — calcurse wrapper with stale PID cleanup.
# Do not edit manually; regenerated on each cmdr launch.
set -uo pipefail

CONF_DIR="${1:-.}"
DATA_DIR="${2:-.}"
PID_FILE="${DATA_DIR}/.calcurse.pid"

# Remove stale PID file if the referenced process is no longer running.
if [ -f "$PID_FILE" ]; then
...
exec calcurse -C "$CONF_DIR" -D "$DATA_DIR"
`

	if err := os.WriteFile(scriptPath, []byte(content), 0o755); err != nil {
		return "", fmt.Errorf("write calcurse wrapper: %w", err)
	}
	return scriptPath, nil
}
...
func WriteLayout(path string, opts LayoutOpts) error {
	...
	// Generate the calcurse-wrapper script for stale PID cleanup.
	if _, err := WriteCalcurseWrapper(scriptDir); err != nil {
		fmt.Fprintf(os.Stderr, "Warning: failed to generate calcurse-wrapper: %v\n", err)
	}
